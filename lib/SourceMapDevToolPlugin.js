...
const assetDataCache = new WeakMap();
const assetFileCache = new WeakMap();

const getTaskForFile = (file, chunk, options, compilation) => {
	const asset = compilation.assets[file];
	if (assetFileCache.get(asset) === file) {
		const data = assetDataCache.get(asset);
			...
			compilation.hooks.afterOptimizeChunkAssets.tap(
				...
				(context, chunks) => {
					...
					tasks.forEach((task, index) => {
						...
						sourceMap.file = file;
						assetDataCache.set(asset, data);
						assetFileCache.set(asset, file);
						...
						if (sourceMapFilename) {
							...
							if (currentSourceMappingURLComment !== false) {
								data[file] = compilation.assets[file] = new ConcatSource(
							...
							data[sourceMapFile] = compilation.assets[
								sourceMapFile
